...
import com.acme.kunde.security.LoginRepository;
...
class KundeWriteServiceTest {
    ...
    private final DelegatingPasswordEncoder passwordEncoder = new DelegatingPasswordEncoder(
        "argon2id",
        Map.of("argon2id", new Argon2PasswordEncoder(16, 32, 1, 1 << 14, 3))
    );
    ...
    void beforeEach() {
        ...
        final CustomUserDetailsService userDetailsService = new CustomUserDetailsService(loginRepo, passwordEncoder);
        mailSender = mock(JavaMailSender.class);
        final MailProps mailProps = mock(MailProps.class);
        final Mailer mailer = new Mailer(mailSender, mailProps);
    ...
    class Erzeugen {
        ...
        void create(final ArgumentsAccessor args) {
            ...
            when(loginRepo.existsByUsername(username)).thenReturn(false);
            ...
            doNothing().when(mailSender).send(isA(MimeMessagePreparator.class));
            ...
            softly.assertThat(kunde.getNachname()).isEqualTo(nachname);
    ...
    class Aendern {
        ...
        void updateVersionOutdated(final ArgumentsAccessor args) {
            ...
            // then
            assertThat(versionOutdatedException).isNotNull();
            assertThat(versionOutdatedException.getVersion()).isEqualTo(version);
        }
    }
    ...
    // -------------------------------------------------------------------------
    private Kunde createKundeMock(final String nachname, final String email, final String plz) {
        return createKundeMock(null, nachname, email, plz);
    }

    private Kunde createKundeMock(final UUID id, final String nachname, final String email, final String plz) {
        final URL homepage;
        try {
