...
import com.acme.kunde.rest.patch.InvalidPatchOperationException;
...
class KundeWriteController {
    ...
    private int getVersion(final Optional<String> version) {
        log.trace("getVersion: {}", version);
        if (version.isEmpty()) {
            throw new VersionInvalidException(PRECONDITION_REQUIRED);
        }
        final var versionStr = version.get();
        if (versionStr.length() < 3 ||
            versionStr.charAt(0) != '"' ||
            versionStr.charAt(versionStr.length() - 1) != '"') {
            throw new VersionInvalidException(versionStr, PRECONDITION_FAILED);
        }
        final int versionInt;
        try {
            versionInt = Integer.parseInt(versionStr.substring(1, versionStr.length() - 1));
        } catch (final NumberFormatException ex) {
            throw new VersionInvalidException(versionStr, PRECONDITION_FAILED, ex);
        }
        log.trace("getVersion: versionInt={}", versionInt);
        return versionInt;
    }
    ...
    ProblemDetail onVersionInvalid(
        ...
        problemDetail.setInstance(uriHelper.getBaseUri(request));
        return problemDetail;
    }
